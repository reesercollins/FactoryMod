package reesercollins.FactoryMod.recipes.scaling;
...
public class ProductionRecipeModifier {
	...
	public void addConfig(int minimumRuns, int maximumRuns, double baseIncrease, double maximumIncrease, int rank) {
		configs.put(rank,
				new ProductionRecipeModifierConfig(minimumRuns, maximumRuns, baseIncrease, maximumIncrease, rank));
	}
	...
	public ProductionRecipeModifier clone() {
		...
		for (ProductionRecipeModifierConfig config : this.configs.values()) {
			modi.addConfig(config.getMinimumRuns(), config.getMaximumRuns(), config.getBaseIncrease(),
					config.getMaximumIncrease(), config.getRank());
		}
		return modi;
	}
	...
	private class ProductionRecipeModifierConfig {
		...
		public double getModifier(int runs) {
			...
			return (((double) (runs - minimumRuns)) / ((double) (maximumRuns - minimumRuns))
					* (maximumIncrease - baseIncrease)) + baseIncrease;
		}
	}
